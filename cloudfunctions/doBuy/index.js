  ...
  try {
    const result = await db.runTransaction(async transaction => {
      ...
      const orderData = {
        ...
        // status: 0, // 0-待确认
        pay_status: payWithBalance || useMiandan || freeOrder ? true : false, // 余额支付/免单/家庭免支付直接标记为已支付，微信支付由回调更新
    ...
    // 5. 余额支付/免单/家庭免支付订单立即打印（微信支付在支付回调中打印）
    if (result.success && result.orderId && (payWithBalance || useMiandan || freeOrder)) {
      try {
