...
Page({
  ...
  updateCanSubmit() {
    ...
    // 桌码校验（家庭免支付模式下无需桌码）
    if (!config.familyFreeMode && !tableNumber) {
      canSubmit = false
    }
  ...
  async submitOrder(e) {
    ...
    // 检查桌码（家庭免支付模式下无需桌码）
    if (!this.data.familyFreeMode && !this.data.tableNumber) {
      wx.showToast({
        title: '请先扫描桌码',
        icon: 'none'
      })
    ...
    // 确定支付方式（家庭免支付模式下强制不走余额/微信，统一按免费下单处理）
    const familyFreeMode = this.data.familyFreeMode
    const payWithBalance = !familyFreeMode && payMethod === 'balance' && !useMiandan
    ...
    try {
      ...
      if (familyFreeMode || payWithBalance || useMiandan) {
        // 家庭免支付/余额支付/免单：云函数已处理完成，订单已是已支付状态
        wx.hideLoading()
